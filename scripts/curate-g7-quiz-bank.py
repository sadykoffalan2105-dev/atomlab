...
CHAPTER_FACTS: dict[int, list[tuple[str, str, list[str], str]]] = {
    ...
    5: [
        ...
        (
            "Что такое Кислота в школьном определении?",
...
def mcq(sid: str, n: int, question: str, correct: str, wrong: list[str], explanation: str) -> dict:
    ...
    if q.endswith("…") or q.endswith("..."):
        q = q.rstrip("….").rstrip()
    if not q.endswith("?"):
        # Prefer full interrogative; definition stems → «Что такое …?»
        m = re.match(r"^(.+?)\s*[—–-]\s*это$", q, re.I)
        if m:
            q = f"Что такое {clean(m.group(1))}?"
        elif re.search(r"\bэто$", q, re.I):
            q = f"Что такое {re.sub(r'\s*это$', '', q, flags=re.I).strip()}?"
        else:
            q = q + "?"
    return {
        "id": qid,
        ...
        "choices": choices,
        "correctIndex": 0,
        "explanation": shorten(explanation, 200).rstrip("…") + ("." if not explanation.rstrip().endswith((".", "!", "?")) else ""),
...
def build_section(sec: dict) -> list[dict]:
    ...
    for c in sec.get("conceptsRu", []):
        ...
        if 25 <= len(c) <= 140 and not is_garbage(c) and not is_weak_answer(c):
            if re.search(r"\b(это|явля|состо|образу|использу|примен|равен|около|называ)\b|[—–]", c, re.I):
                add(f"Какое утверждение верно по теме «{shorten(topic, 48).rstrip('…')}»?", c, c)
    ...
    for line in sec.get("rememberRu", "").split("\n"):
        ...
        if re.search(r"производится|кисловатый|имеются природные и синтетические типы", line, re.I):
            continue
        # Prefer concrete question from definition shape
        qa = parse_definition(line)
        if qa:
            add(f"Что такое {qa[0]}?", qa[1], line)
        else:
            add(f"Какое утверждение верно по теме «{shorten(topic, 48).rstrip('…')}»?", line, line)
    ...
    for sent in re.split(r"(?<=[.!?])\s+", sec.get("contentRu", "")):
        ...
        if not re.search(r"\b(это|является|называ|состоит|образу|примен|использу|равен|около)\b", s, re.I):
            continue
        add(f"Какое утверждение верно по теме «{shorten(topic, 48).rstrip('…')}»?", s, s)
    ...
    # No meta pads about «учебник / проверить знания» — only chemistry content.
