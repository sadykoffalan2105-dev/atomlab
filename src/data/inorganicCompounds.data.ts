...
function saltDescriptionRu(id: string, cat: CationDef, anKey: string): string {
  ...
  if (hal) {
    ...
    return pick(
      `${title} — простая ионная соль галогенид-аниона. В школьных опытах растворы используют для реакций обмена и качественных реакций на галогенид-ионы (AgCl, AgBr, AgI).`,
      `${title} в кристаллическом виде — ионная решётка; в растворе (если растворим) — электролит. На уроках сравнивают активность металлов и осаждение.`,
      `${title} — наглядный пример ионного соединения. Для хлоридов/бромидов/иодидов часто показывают осаждение с AgNO₃.`,
    )
  }
  if (anKey === 'NO3') {
    return pick(
