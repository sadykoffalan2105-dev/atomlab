...
export function prepareTextForHumanTts(
  ...
  if (locale === 'ru') {
    t = t
      ...
      .replace(/(?<!\p{L})стр\.?\s*(\d+)/giu, 'страница $1')
      ...
      // \b не работает с кириллицей — границы через \p{L}; точки обязательны («те» ≠ «т. е.»)
      .replace(/(?<!\p{L})и\s+т\.\s*[дп]\./giu, 'и так далее')
      .replace(/(?<!\p{L})т\.\s*д\./giu, 'так далее')
      .replace(/(?<!\p{L})т\.\s*е\./giu, 'то есть')
      .replace(/…+/g, '.')
  } else if (locale === 'uz') {
    t = t
