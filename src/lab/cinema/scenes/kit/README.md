...
Набор, на котором собираются сцены синтеза: `co2`, `h2o`, `nh3`, `so2`, …
Он снимает с автора сцены всё, что одинаково, и оставляет ему только химию и раскадровку.

**Исключение — `scenes/nacl/*`**: сцена NaCl вынесена во фреймворк-независимый класс `NaClReactionScene` (three + GSAP, свой свет и материалы, передача кадра герою продукта через `hero/heroHandoff`); `NaclCinemaScene.tsx` — тонкий R3F-адаптер без SceneShell. Числа химии и тексты урока — по тем же правилам, что ниже.
